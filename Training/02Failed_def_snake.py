...
def take_current_snake_row_column(matrix):
    snake_row = 0
    snake_column = 0
...
def up_movement(snake_row, snake_column):
        ...
        if is_valid(current_row, current_column):
            if matrix[current_row][current_column] == FOOD:
                find_food(current_row, current_column)
...
def find_food(current_row, current_column):
    if matrix[current_row][current_column] == FOOD:
        return food_quantity + 1

def find_barrows(current_row, current_column):
    if matrix[current_row][current_column] == BURROWS:
...
while in_territory:
    command = input()
    take_current_snake_row_column(matrix)
    snake_row, snake_column = take_current_snake_row_column(matrix)
    if command == UP_COMMAND:
        up_movement(snake_row, snake_column)
